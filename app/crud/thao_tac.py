...
from schemas import ThaoTacBase, ThaoTac


def create_thao_tac(db: Session, thao_tac: ThaoTac):
...
def delete_thao_tac(db: Session, thao_tac_id: str = None, thao_tac_name: str = None):
    """
...
def get_thao_tac(db: Session, thao_tac_id: str = None, thao_tac_name: str = None):
    if thao_tac_id:
        db_thao_tac = db.query(Thao_tac).filter(Thao_tac.id == thao_tac_id).first()
    elif thao_tac_name:
        db_thao_tac = db.query(Thao_tac).filter(Thao_tac.Ten_thao_tac.ilike(f"%{thao_tac_name}%")).all()
...
def get_all_thao_tac(db: Session):
    ...
    """
    return db.query(Thao_tac).order_by(Thao_tac.id).all()

def update_thao_tac(db: Session, thao_tac_id: str, thao_tac: ThaoTacBase):
    db_thao_tac = db.query(Thao_tac).filter(Thao_tac.id == thao_tac_id).first()
    if db_thao_tac is None:
        raise HTTPException(status_code=404, detail="Thao tác không tồn tại")
